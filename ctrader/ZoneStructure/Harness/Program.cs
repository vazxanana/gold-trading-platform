...
internal static class Program
{
    ...
    public static bool TryPocketBand(Smc.StructureMap map, List<Smc.Candle> h4c, out double bandLo, out double bandHi, out int legId)
    {
        ...
        if (st.Trend == "bear")
        {
            ...
            for (int k = pos; k < h4c.Count; k++) legLow = Math.Min(legLow, h4c[k].Low);
        ...
        else
        {
            ...
            for (int k = pos; k < h4c.Count; k++) legHigh = Math.Max(legHigh, h4c[k].High);
            double range = legHigh - legLow;
            if (range <= 0) return false;
    ...
    public static (List<Entry> entries, int triggersNoZone, int zonesArmed, int alignBlocked) Simulate(List<Bar> m15raw, int h4Look = H4Lookback, int d1Look = D1Lookback, int m15Look = M15Lookback)
    {
        ...
        for (int m = SwingStrength * 2 + 6; m < m15raw.Count - 1; m++)
        {
            ...
            foreach (var z in h4Map.Zones)
            {
                ...
                if (!touched) continue;
                if (UseFibFilter && !InPocket(h4Map, h4Candles, z)) continue;
